	...
	function init() {
	 ...
	 bodyDef.type = b2Body.b2_staticBody;
	 bodyDef.position.x = homeX - worldWidth/2;
	 bodyDef.position.y = worldHeight / 2;
	 fixDef.shape = new b2PolygonShape;
	 fixDef.shape.SetAsBox(2.5, worldHeight);
	 world.CreateBody(bodyDef).CreateFixture(fixDef);
	...
	function createIngredient(definition)
	{
		...
		bodyDef.angle = Math.PI / 4 + Math.random() * .05;
	...
	function MakeIce()	{		createIngredient(ingredients.ice);	}
	function MakeStrawberry()	{		createIngredient(ingredients.strawberry);	}
